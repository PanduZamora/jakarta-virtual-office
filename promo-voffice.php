<!doctype html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <title>Promo vOffice</title>
    <meta name="description" content="Promo on vOffice Indonesia" />
    <meta name="robots" content="index,follow" />
    <meta name="Author" content="vOffice.co.id">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="css/font-awesome.min.css" rel="stylesheet" type="text/css" media="all">
    <link href="css/themify-icons.css" rel="stylesheet" type="text/css" media="all" />
    <link href="css/bootstrap.css" rel="stylesheet" type="text/css" media="all" />
    <link href="css/flexslider.css" rel="stylesheet" type="text/css" media="all" />
    <link href="css/lightbox.min.css" rel="stylesheet" type="text/css" media="all" />
    <link href="css/theme-v2.css" rel="stylesheet" type="text/css" media="all" />
    <link href="css/custom-new.min.css" rel="stylesheet" type="text/css" media="all" />
    <link href='https://fonts.googleapis.com/css?family=Lato:300,400%7CRaleway:100,400,300,500,600,700%7COpen+Sans:400,500,600' rel='stylesheet' type='text/css'>
    <link href="https://fonts.googleapis.com/css?family=Montserrat:100,300,400,600,700" rel="stylesheet" type="text/css">
    <link href="css/extra.min.css" rel="stylesheet" type="text/css" media="all" />

    <link href="promo-voffice/style-promo.css" rel="stylesheet" type="text/css" media="all" />
    <!--slide carousel-->
    <link href="css/slick/slick/slick.css" rel="stylesheet" type="text/css" />
    <link href="css/slick/slick/slick-theme.css" rel="stylesheet" type="text/css" />

    <style media="screen">
        .header-overlay {
            height: 140px;
            background-color: rgba(14, 14, 14, 0.85)
        }

        .owl-dots {
            display: none !important;
        }
    </style>
</head>

<body>
    <?php include "header.html" ?>
    <div class="header-overlay">

    </div>
    <div class="main-container">
        <section class="section-header-promo bg-header-promo">
            <div class="row">
                <h1 class="bold mb0 text-center"> Special Promo</h1>

                <p class="text-center mb40">(Promo valid until <?php echo date('t F Y'); ?>) </p>

            </div>

        </section>

        <section>
            <div class="container">

                <div class="row p50">
                    <h2 class="bold text-center"> Virtual Office</h2>
                    <div class="hr center mb20-lg mb10-xs"></div>
                </div>

                <div class="row">

                    <!-- Cabang New PIK -->
                    <div class="col-md-3">
                        <div class="image-tile hover-tile text-center mb20">
                            <a data-toggle="#" data-target="#" href="promo-voffice/virtual-office-promo-spesial-bonus.php">
                                <img class="background-image" src="images/promo/juli/special-bonus.jpg">
                            </a>
                            <div class="row">
                                <div class="col-sm-12" style="padding-right: 2px;">
                                    <a class="btn btn-lg btn-block border-radius-0 btn-filled color-white m0" data-toggle="#" data-target="#" href="promo-voffice/virtual-office-promo-6.php">Contact us</a>
                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Union Space Thailan -->
                    <div class="col-md-3">
                        <a href="unionspace-thailand.php" class="slide-promo">
                            <div>
                                <img src="images/promo/union-space-1.png" alt="" class="img-fluid">
                            </div>
                            <div>
                                <img src="images/promo/union-space-2.png" alt="" class="img-fluid">
                            </div>
                            <div>
                                <img src="images/promo/union-space-3.png" alt="" class="img-fluid">
                            </div>
                            <div>
                                <img src="images/promo/union-space-4.png" alt="" class="img-fluid">
                            </div>
                        </a>
                        <a class="btn btn-lg btn-block border-radius-0 btn-filled color-white m0" href="unionspace-thailand.php">See more</a>

                    </div>

                    <!-- Cabang New PIK -->
                    <div class="col-md-3">
                        <div class="image-tile hover-tile text-center mb20">
                            <a data-toggle="#" data-target="#" href="promo-voffice/virtual-office-pik.php">
                                <img class="background-image" src="images/promo/mei/gold-coast.png">
                            </a>
                            <div class="row">
                                <div class="col-sm-12" style="padding-right: 2px;">
                                    <a class="btn btn-lg btn-block border-radius-0 btn-filled color-white m0" data-toggle="#" data-target="#" href="promo-voffice/virtual-office-promo-6.php">Contact us</a>
                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Special Price + Extra 1 Month ( Sale - Bretion ) -->
                    <div class="col-md-3">
                        <div class="image-tile hover-tile text-center mb20">
                            <a data-toggle="#" data-target="#" href="promo-voffice/virtual-office-promo-6.php">
                                <img class="background-image" src="images/promo/mei/atria-sudirman.png">
                            </a>
                            <div class="row">
                                <div class="col-sm-12" style="padding-right: 2px;">
                                    <a class="btn btn-lg btn-block border-radius-0 btn-filled color-white m0" data-toggle="#" data-target="#" href="promo-voffice/virtual-office-promo-6.php">Contact us</a>
                                </div>
                            </div>
                        </div>
                    </div>



                </div>

                <div class="row">

                    <!-- Sprecial Price + Extra 1 Month ( Sale - Bretion ) -->
                    <div class="col-md-3">
                        <div class="image-tile hover-tile text-center mb20">
                            <a data-toggle="#" data-target="#" href="promo-voffice/virtual-office-promo.php">
                                <img class="background-image" src="images/promo/juli/july_fair.png">
                            </a>
                            <div class="row">
                                <div class="col-sm-12" style="padding-right: 2px;">
                                    <a class="btn btn-lg btn-block border-radius-0 btn-filled color-white m0" data-toggle="#" data-target="#" href="promo-voffice/virtual-office-promo.php">Contact us</a>
                                </div>
                            </div>
                        </div>
                    </div>


                    <!-- Sprecial Price + Extra 1 Month ( Sale - Bretion ) -->
                    <div class="col-md-3">
                        <div class="image-tile hover-tile text-center mb20">
                            <a data-toggle="#" data-target="#" href="promo-voffice/virtual-office-promo-1.php">
                                <img class="background-image" src="images/promo/maret/pindah-vo-12-bulan.jpg">
                            </a>
                            <div class="row">
                                <div class="col-sm-12" style="padding-right: 2px;">
                                    <a class="btn btn-lg btn-block border-radius-0 btn-filled color-white m0" data-toggle="#" data-target="#" href="promo-voffice/virtual-office-promo-1.php">Contact us</a>
                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Sprecial Price + Extra 1 Month ( Sale - Bretion ) -->
                    <div class="col-md-3">
                        <div class="image-tile hover-tile text-center mb20">
                            <a data-toggle="#" data-target="#" href="promo-voffice/virtual-office-promo-2.php">
                                <img class="background-image" src="images/promo/maret/coworking-promo.jpg">
                            </a>
                            <div class="row">
                                <div class="col-sm-12" style="padding-right: 2px;">
                                    <a class="btn btn-lg btn-block border-radius-0 btn-filled color-white m0" data-toggle="#" data-target="#" href="promo-voffice/virtual-office-promo-2.php">Contact us</a>
                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Sprecial Price + Extra 1 Month ( Sale - Bretion ) -->
                    <div class="col-md-3">
                        <div class="image-tile hover-tile text-center mb20">
                            <a data-toggle="#" data-target="#" href="promo-voffice/virtual-office-promo-3.php">
                                <img class="background-image" src="images/promo/maret/sign-up-vo-free.jpg">
                            </a>
                            <div class="row">
                                <div class="col-sm-12" style="padding-right: 2px;">
                                    <a class="btn btn-lg btn-block border-radius-0 btn-filled color-white m0" data-toggle="#" data-target="#" href="promo-voffice/virtual-office-promo-3.php">Contact us</a>
                                </div>
                            </div>
                        </div>
                    </div>



                </div>

                <div class="row">

                    <!-- MRT -->
                    <div class="col-md-3">
                        <div class="image-tile hover-tile text-center mb20">
                            <a data-toggle="#" data-target="#" href="promo-voffice/virtual-office-promo-4.php">
                                <img class="background-image" src="images/promo/Meeting MRT REV 5.jpg">
                            </a>
                            <div class="row">
                                <div class="col-sm-12" style="padding-right: 2px;">
                                    <a class="btn btn-lg btn-block border-radius-0 btn-filled color-white m0" data-toggle="#" data-target="#" href="promo-voffice/virtual-office-promo-4.php">Contact us</a>
                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Braga Bandung -->
                    <div class="col-md-3">
                        <div class="image-tile hover-tile text-center mb20">
                            <a data-toggle="#" data-target="#" href="promo-voffice/virtual-office-promo-5.php">
                                <img class="background-image" src="images/promo/maret/vo-braga-bandung.jpg">
                            </a>
                            <div class="row">
                                <div class="col-sm-12" style="padding-right: 2px;">
                                    <a class="btn btn-lg btn-block border-radius-0 btn-filled color-white m0" data-toggle="#" data-target="#" href="promo-voffice/virtual-office-promo-5.php">Contact us</a>
                                </div>
                            </div>
                        </div>
                    </div>

                </div>


                <div class="row pt96 pb56">
                    <h2 class="bold text-center"> Serviced Office Promo</h2>
                    <div class="hr center mb20-lg mb10-xs"></div>
                </div>


                <div class="row">

                    <!-- Serviced Office -->
                    <div class="col-md-3">
                        <div class="image-tile hover-tile text-center mb20">
                            <a data-toggle="#" data-target="#" href="promo-voffice/serviced-office-promo.php">
                                <img class="background-image" src="images/promo/Promo SO April.jpg">
                            </a>
                            <div class="row">
                                <div class="col-sm-12" style="padding-right: 2px;">
                                    <a class="btn btn-lg btn-block border-radius-0 btn-filled color-white m0" data-toggle="#" data-target="#" href="promo-voffice/serviced-office-promo.php">Contact us</a>
                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Grand Jati Junction 2jt -->
                    <div class="col-md-3">
                        <div class="image-tile hover-tile text-center mb20">
                            <a data-toggle="#" data-target="#" href="promo-voffice/serviced-office-promo-1.php">
                                <img class="background-image" src="images/promo/maret/promo-so-medan.png">
                            </a>
                            <div class="row">
                                <div class="col-sm-12" style="padding-right: 2px;">
                                    <a class="btn btn-lg btn-block border-radius-0 btn-filled color-white m0" data-toggle="#" data-target="#" href="promo-voffice/serviced-office-promo-1.php">Contact us</a>
                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Bumi Mandiri Tower II -->
                    <div class="col-md-3">
                        <div class="image-tile hover-tile text-center mb20">
                            <a data-toggle="#" data-target="#" href="promo-voffice/serviced-office-promo-2.php">
                                <img class="background-image" src="images/promo/bumi-mandiri-tower-ii.jpeg">
                            </a>
                            <div class="row">
                                <div class="col-sm-12" style="padding-right: 2px;">
                                    <a class="btn btn-lg btn-block border-radius-0 btn-filled color-white m0" data-toggle="#" data-target="#" href="promo-voffice/serviced-office-promo-2.php">Contact us</a>
                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Home Business Package -->
                    <div class="col-md-3">
                        <div class="image-tile hover-tile text-center mb20">
                            <a data-toggle="#" data-target="#" href="promo-voffice/serviced-office-promo-3.php">
                                <img class="background-image" src="images/promo/mei/so-intiland.png">
                            </a>
                            <div class="row">
                                <div class="col-sm-12" style="padding-right: 2px;">
                                    <a class="btn btn-lg btn-block border-radius-0 btn-filled color-white m0" data-toggle="#" data-target="#" href="promo-voffice/serviced-office-promo-3.php">Contact us</a>
                                </div>
                            </div>
                        </div>
                    </div>

                </div>

                <div class="row pt96 pb56">
                    <h2 class="bold text-center"> Other Facilities Promo</h2>
                    <div class="hr center mb20-lg mb10-xs"></div>
                </div>

                <div class="row">

                    <!-- Jurnal -->
                    <div class="col-md-3">
                        <div class="image-tile hover-tile text-center mb20">
                            <a data-toggle="#" data-target="#" href="promo-voffice/facilities-promo.php">
                                <img class="background-image" src="images/promo/Jurnal_by_Mekari.png">
                            </a>
                            <div class="row">
                                <div class="col-sm-12" style="padding-right: 2px;">
                                    <a class="btn btn-lg btn-block border-radius-0 btn-filled color-white m0" data-toggle="#" data-target="#" href="promo-voffice/facilities-promo.php">Contact us</a>
                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Grand Jati Junction 4,5jt -->
                    <div class="col-md-3">
                        <div class="image-tile hover-tile text-center mb20">
                            <a data-toggle="#" data-target="#" href="promo-voffice/facilities-promo-1.php">
                                <img class="background-image" src="images/promo/Promo SO Medan Agustus REV1.jpg">
                            </a>
                            <div class="row">
                                <div class="col-sm-12" style="padding-right: 2px;">
                                    <a class="btn btn-lg btn-block border-radius-0 btn-filled color-white m0" data-toggle="#" data-target="#" href="promo-voffice/facilities-promo-1.php">Contact us</a>
                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Grand Jati Junction - Medan -->
                    <div class="col-md-3">
                        <div class="image-tile hover-tile text-center mb20">
                            <a href="serviced-office-medan.php">
                                <img class="background-image" src="images/promo/mei/package-room-grand-jati-junction.png">
                            </a>
                            <a class="btn btn-lg btn-filled m0 border-radius-0 btn-block color-white" href="serviced-office-medan.php">See More</a>
                        </div>
                    </div>

                </div>
            </div>
        </section>


        <section>
            <h1 class="bold mb16 text-center ls"> Our New Centre </h1>
            <p class="text-center mb56">Dapatkan Harga Special & Penawaran Menarik dari vOffice</p>
            <div class="container">
                <div class="row">

                    <div class="col-md-4 mb40">
                        <img src="images/lokasi-gedung-voffice/pik/pik.png" alt="" height="300" style="object-fit:cover; width:100%;">
                        <h4 class="bold" style="margin-bottom:4px;margin-top:10px;">Gold Coast Office</h4>
                        <p class="mb16">
                            Gold Coast Office Eiffel Tower Lt 12 Jl Pantai Indah Kapuk, Penjaringan Jakarta Utara
                        </p>
                        <a href="virtual-office-pik.php" class="btn btn-filled mb-xs-32">See More</a>
                    </div>

                    <div class="col-md-4 mb40">
                        <img src="images/lokasi-gedung-voffice/tanah-abang/tanah-abang.png" alt="" height="300" style="object-fit:cover; width:100%;">
                        <h4 class="bold" style="margin-bottom:4px;margin-top:10px;">Atria Sudirman</h4>
                        <p class="mb16">
                            Atria Sudirman Lt 23 Jl Jend Sudirman Kav 33A, Karet Tengsin Tanah Abang Jakarta Pusat
                        </p>
                        <a href="virtual-office-tanah-abang.php" class="btn btn-filled mb-xs-32">See More</a>
                    </div>

                    <div class="col-md-4 mb40">
                        <img src="img/tower/distrik8/treasury_tower_new.png" alt="" height="300" style="object-fit:cover; width:100%;">
                        <h4 class="bold" style="margin-bottom:4px;margin-top:10px;">Treasury Tower</h4>
                        <p class="mb16">JL. JEND. SUDIRMAN KAV. 52-53
                            SUDIRMAN CENTRAL BUSINESS DISTRICT (SCBD), Jakarta Selatan
                        </p>
                        <a href="treasury-tower.php" class="btn btn-filled mb-xs-32">See More</a>
                    </div>


                </div>

                <div class="row">

                    <div class="col-md-4 mb40">
                        <img src="img/bandung/braga_bandung_1.png" width="100%" height="300" style="object-fit:cover; width:100%;">
                        <h4 class="bold" style="margin-bottom:4px;margin-top:10px;">Braga Bandung</h4>
                        <p class="mb16">Jalan Braga Nomor 109, Kecamatan Sumur, Kota Bandung, Jawa Barat 40111
                        </p>
                        <a href="virtual-office-bandung.php" class="btn btn-filled mb-xs-32">See More</a>
                    </div>

                    <div class="col-md-4 mb40">
                        <img src="images/kemayoran/kemayoran-2.jpg" alt="" height="300" style="object-fit:cover; width:100%;">
                        <h4 class="bold" style="margin-bottom:4px;margin-top:10px;">Citra Tower </h4>
                        <p class="mb16">Blok A6, Jl. Benyamin Suaeb, Kemayoran, Kota Jakarta
                        </p>
                        <a href="virtual-office-kemayoran.php" class="btn btn-filled mb-xs-32">See More</a>
                    </div>

                    <div class="col-md-4 mb40">
                        <img src="images/sudirman.jpg" alt="" height="300" style="object-fit:cover; width:100%;">
                        <h4 class="bold" style="margin-bottom:4px;margin-top:10px;">Sudirman 7.8 Tower </h4>
                        <p class="mb16">Level 16 unit 1 & 2, Jl. Jend. Sudirman No.Kav 7-8, Jakarta Pusat
                        </p>
                        <a href="virtual-office-sudirman-78.php" class="btn btn-filled mb-xs-32">See More</a>
                    </div>

                </div>
                <div class="row">


                    <div class="col-md-4 mb40">
                        <img src="images/medan/grand-jati-junc-square.jpg" alt="" height="300" style="object-fit:cover; width:100%;">
                        <h4 class="bold" style="margin-bottom:10px;margin-top:10px;">Grand Jati Junction</h4>
                        <p class="mb16">Jl. Perintis Kemerdekaan No.3A, Kec. Medan Tim., Kota Medan
                        </p>
                        <a href="virtual-office-medan.php" class="btn btn-filled mb-xs-32">See More</a>
                    </div>

                    <div class="col-md-4 mb40">
                        <img src="images/lokasi-gedung-voffice/jakarta utara/honey-lady.jpg" alt="" height="300" style="object-fit:cover; width:100%;">
                        <h4 class="bold" style="margin-bottom:10px;margin-top:10px;">The Honey Lady</h4>
                        <p class="mb16">Level 2, Jl. Pluit Selatan Raya No.1, Penjaringan, Jakarta Utara</p>
                        <a href="virtual-office-pluit.php" class="btn btn-filled mb-xs-32">See More</a>
                    </div>

                </div>
            </div>
        </section>
        <?php include "footer-vo.html" ?>


        <!-- Default -->
        <div aria-hidden="true" aria-labelledby="myModalLabel" class="modal fade" id="enquire" role="dialog" tabindex="-1">
            <div class="modal-dialog modal-xs mb-xs-64">
                <div class="modal-content">
                    <!-- Modal Header -->
                    <div class="modal-header clearfix">
                        <button class="close" data-dismiss="modal" type="button">
                            <span aria-hidden="true">
                                ×
                            </span>
                            <span class="sr-only">
                                Close
                            </span>
                        </button>
                        <h4 class="mb8 bold text-center color-black">Dapatkan Promo & Penawaran Menarik</h4>
                        <center>Isi formulir di bawah ini untuk mendapatkan proposal penawaran
                            dan tim kami akan segera menghubungi Anda. Terima kasih
                        </center>
                    </div>
                    <div class="modal-body form-body clearfix">
                        <form class="form-email" data-success="Thanks for your inquiries, we will be in touch to confirm shortly." data-error="Please fill all fields correctly.">
                            <input type="text" name="promo_so" hidden="hidden" value="voffice-promo" id="voffice-promo">
                            <input type="text" class="validate-required" name="name" placeholder="Nama Lengkap*">
                            <input type="text" class="validate-required" name="phone" placeholder="Nomor Telepon / Whatsapp*">
                            <input type="text" class="validate-required validate-email" name="email" placeholder="Alamat Email*">
                            <input type="text" class="" name="usaha" placeholder="Bidang Usaha">
                            <textarea style="color: #0a0a0a" name="message" rows="4" placeholder="Message"></textarea>
                            <button id="submit-form" type="submit" name="submit">Contact Us Now!</button>
                        </form>
                    </div>
                </div>
            </div>
        </div>


    </div>

    <!-- Whatsapp Float -->
    <a href="http://bit.ly/voffice-webnew" class="back-to-top link-changer visible-xs" id="waFloatButton" target="_blank">
        <i class="fab fa fa-whatsapp ">
        </i>
    </a>


    <script src="js/jquery.min.js"> </script>
    <script src="js/bootstrap.min.js"></script>
    <script src="js/validator.js"></script>
    <script src="js/flexslider.min.js"></script>
    <script src="js/lightbox.min.js"></script>
    <script src="js/masonry.min.js"></script>
    <script src="js/smooth-scroll.min.js"></script>
    <script src="js/scripts.js"></script>
    <script src="css/slick/slick/slick.js"></script>
    <!--slide-->

    <script>
        $('.slide-promo').slick({
            infinite: true,
            autoplay: true,
            slidesToShow: 1,
            slidesToScroll: 1
        });
    </script>

    <script>
        const field = $(".field-changer");
        if (link.includes("?fbigads")) {
            $('.text-changer').html('<i class="fab fa fa-whatsapp fa-lg mr-10"></i> Whatsapp');
            $('.link-changer').attr('href', ' https://bit.ly/fbigadsvoffice');
            field.attr('value', 'virtualoffice_fbigads');
            field.attr('name', 'virtualoffice_fbigads');
            console.log(field)
        } else if (link.includes("?tiktokads")) {
            $('.text-changer').html('<i class="fab fa fa-whatsapp fa-lg mr-10"></i> Whatsapp');
            $('#wa-fb-ig').css("display", "none");
            $('#wa-fb-ig2').css("display", "none");
            $('#wa-fb-ig3').css("display", "none");
            $('#wa-fb-ig4').css("display", "none");
            $('#wa-fb-ig5').css("display", "none");
            $('#wa-fb-ig6').css("display", "none");
            $('#wa-fb-ig7').css("display", "none");
            $('.link-changer').attr('href', ' https://bit.ly/tiktokadsvoffice');
            field.attr('value', 'virtualoffice_tiktokads');
            field.attr('name', 'virtualoffice_tiktokads');
            console.log(field)
        } else if (link.includes("?sem")) {
            $('.text-changer').html('<i class="fab fa fa-whatsapp fa-lg mr-10"></i> Whatsapp');
            $('#wa-fb-ig').css("display", "none");
            $('#wa-fb-ig2').css("display", "none");
            $('#wa-fb-ig3').css("display", "none");
            $('#wa-fb-ig4').css("display", "none");
            $('#wa-fb-ig5').css("display", "none");
            $('#wa-fb-ig6').css("display", "none");
            $('#wa-fb-ig7').css("display", "none");
            $('.link-changer').attr('href', ' http://bit.ly/sem-voffice');
            field.attr('value', 'virtualoffice_sem');
            field.attr('name', 'virtualoffice_sem');
            console.log(field)
        } else if (link.includes("")) {
            $('#wa-fb-ig').css("display", "none");
            $('#wa-fb-ig2').css("display", "none");
            $('#wa-fb-ig3').css("display", "none");
            $('#wa-fb-ig4').css("display", "none");
            $('#wa-fb-ig5').css("display", "none");
            $('#wa-fb-ig6').css("display", "none");
            $('#wa-fb-ig7').css("display", "none");
            console.log(field)
        }
    </script>

</body>

</html>