...
if ($_POST["gopay_x"])
{
			...
			$vo_inv_db = "client_invoices_".$invoices1;
			$q_inv = "SELECT currency, setup_fee, amount_due FROM $vo_inv_db WHERE invoice_id='$invoices2' AND invoice_status='1'";
			$q_inv_info = mysql_query ($q_inv, $xdbm) or die (mysql_error());			
			...
			if ($f_currency == "RM")
			{
				...
				$curencytype = $f_currency;
			}
			
			if ($pinfo_qrow[1] != 0.00)
			{
				$raw_total = $pinfo_qrow[1] + $pinfo_qrow[2];
			}
			else
			{
				$raw_total = $pinfo_qrow[2];
			}


			// amount to pay in hidden field
			//$raw_total = $_POST["showamt"];

			/*echo "<script language=\"javascript\"> alert('$raw_total')</script>";
			exit(0);*/
		...
		//{
...

			{
            	  ...
            	  $result = mysql_query($exratesnow, $dbmrates) or die( 'error with exchange rates '.mysql_error());
		 ...
		 //$pprice = $totalpayable * 100;
		  ...
		  $ptotalpayable = $raw_total * $frates;
			...
			$link = EWAYOK;
			  ...
			  // Set the TEST payment details
			  //$eway = new EwayPaymentLive($eWAY_TESTCustomerID, $eWAY_PaymentMethod, $eWAY_TESTUseLive);
			  ...
			  $eway->setTransactionData("CustomerInvoiceDescription", $invDesc);
...

			{
				...
				echo $gw_resp_code = $ewayResponseFields["EWAYTRXNERROR"];
			...
			{			
				...
				$subject_in = "vOffice.com.my VOPay CC Payment : Ref : ". $ref_id . " - Fail";
			...
			else
			{
				...
				$subject_in = "vOffice.com.my VOPay Credit Card Payment : Ref : ". $ref_id;
				...
				$client_subject_in = "vOffice.com.my VOPay Credit Card Payment : Ref : ". $ref_id . " - Order Success";
				...
				$update_inv_stmt = "UPDATE ".$vo_inv_db." SET date_paid='$datepaid',paid_reference='$ref_id', amount_paid='$raw_total', invoice_status='2' WHERE invoice_id = '$invoices2'";
				$update_inv_result = mysql_query ($update_inv_stmt, $xdbm) or die("Could not update Invoice \n");

				// Create Receipt
				$rcpt_table = "client_receipt_".$invoices1;				
				$addreceiptstmt = "INSERT INTO ".$rcpt_table."	
							(vo_id, invoice_id, amount_paid, receipt_date, staff) 
							VALUES 
							('$invoices1', '$invoices2', '$raw_total', '$datepaid','0')";
					
				$resultaddreceiptstmt = mysql_query ($addreceiptstmt, $xdbm) or die (mysql_error());

				$optype = 2; // Apply Payment
		
				// log staff
				$addinvstmt = "INSERT INTO invoice_transaction_log 	
							(vo_id, invoice_id, optype, staff) 
							VALUES 
							('$invoices1', '$invoices2', '$optype', '0')";
					
				$resultaddinvstmt = mysql_query ($addinvstmt, $xdbm) or die (mysql_error());
...
<script type="text/javascript">	
		...
		$(function()
		{
				$("#invno1").change(function() {
					...
					var ininv1 = $("#invno1").val();
					var ininv2 = $("#invno2").val();

					rplan = 'inv1='+ininv1+'&inv2='+ininv2;
					//alert(ininv1);
					//$.get("getInvoiceCurrency.php?"+rplan, function(data){
					//	$("#showcurrency").val(data);
					//});
					$("#showcurrency").load("getInvoiceCurrency.php?"+rplan);  
					$('#showcurrency').html('<p><div align="left"><img src="./images/ajax-loader.gif" /></div></p>');

					$("#showdisamt").load("getInvoiceDueAmt.php?"+rplan);  
					$('#showdisamt').html('<p><div align="left"><img src="./images/ajax-loader.gif" /></div></p>');

					$.get("getInvoiceDueAmt.php?"+rplan, function(data){
						$("#showamt").val(data);
					});
					
				}); 
				
				$("#invno2").change(function() {
...
<div id="showcase_clip"> 
  ...
  <div id="showcase"> 
    ...
    <div class="col1"> 
      ...
      <form name="billpay" method="post" action="<?=$_SERVER['PHP_SELF'];?>"  onSubmit="return validateform()">
          ...
          <table border="0" width="100%">
            ...
            <tbody>
                  ...
                  <?php
							...
							/*	if ($_REQUEST["inv"] != "")
...

								{
                              		...
                              		echo "<input class=\"form_txt\" size=\"8 \" maxlength=\"8\" name=\"invno2\">";
								...
								?>
								     <input class="form_txt" size="3" maxlength="3" name="invno1" id="invno1"> - 
                              		 <input class="form_txt" size="8" maxlength="8" name="invno2" id="invno2">

									 <br><em>(Kindly enter the Invoice Number you're making payment for here if you have one)</em>
              ...
              <tr> 
                ...
                <td id="showdisamt"><input type="hidden" name="showamt" id="showamt" ></td>
...
<table border="0" width="98%" class="noborder">
            ...
            <tbody>
              ...
              <tr> 
				...
				<input name="gopay" id="gopay" type="image" value="" src="./images/proceedpayment.png"/>  
    ...
    <div class="col2"> 
      ...
      <p>If your invoice number or quote number is not accepted by the system (the amount & currency) do not automatically appear, please use this <a href="custompay.php">Secure Online Payment Form</a> instead</p>
