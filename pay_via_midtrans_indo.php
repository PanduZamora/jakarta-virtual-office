...
<script type="text/javascript" src="./midtrans/jquery.fancybox.pack.js"></script><head>
  <link rel="stylesheet" href="./midtrans/veritrans-php-master/examples/vt-direct/jquery.fancybox.css">
			...
			Veritrans_Config::$clientKey = "Mid-client-4BlC444gvNobfJOs";
...
<pre>
  <form action="token-checkout_midtrans.php" method="POST" id="payment-form" hidden="hidden">
        ...
        <input class="card-cvv" value="<?=$_POST['cvc']?>" size="4" type="text" autocomplete="off"  hidden="hidden"/>
        <input type="checkbox" name="secure" value="false" checked hidden="hidden">
        <input type="checkbox" name="save_cc" value="true" checked hidden="hidden">
		<input name="ref" id="ref" value="<?=$ref_id?>"  hidden="hidden"/>
     	<input class="card-price" name="price" id="price" value="<?=$_SESSION['total_price']?>"  hidden="hidden"/>
		...
		<input name="autocharge" id="autocharge" value="<?=$_POST['autocharge']?>"  hidden="hidden"/>
